...
function generateOrg3Definition() {
  ...
  set -x
  configtxgen -printOrg Org3MSP > ../organizations/peerOrganizations/org3.dubu.com/org3.json
  res=$?
  { set +x; } 2>/dev/null
  if [ $res -ne 0 ]; then
...
function Org3Up () {
  ...
  if [ "$CONTAINER_CLI" == "podman" ]; then
    cp ../podman/core.yaml ../../organizations/peerOrganizations/org3.dubu.com/peers/peer0.org3.dubu.com/
  fi
...
function addOrg3 () {
  ...
  if [ ! -d "../organizations/peerOrganizations/org3.dubu.com" ]; then
    generateOrg3
